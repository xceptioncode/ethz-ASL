...
public class RunMW {
	...
	public static void main(String[] args) throws Exception {
		 ...
		 Runtime.getRuntime().addShutdownHook(new Thread() {
            ...
            public void run() {
				try{
				histWriter = new PrintWriter(new FileWriter("HistResponseTime.log"));
                histWriter.println(Arrays.toString(asl.workerThread.responseTimeHist.toArray()));
				histWriter.flush();
            } catch (IOException e) {
			e.printStackTrace();
			}}
		...
		new netThread(myIp, myPort, mcAddresses, numThreadsPTP, readSharded).run();
	...
	private static void parseArguments(String[] args) {
		...
		if (numThreadsPTP > 128) { System.out.println("A maximum of only 128 threads are allowed"); System.exit(1); }}
		else {
